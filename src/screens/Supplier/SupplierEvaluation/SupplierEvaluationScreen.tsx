...
const SupplierEvaluationScreen: React.FC = () => {
  // 模拟供应商评价数据
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.section}>
        <View style={styles.statsContainer}>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statValue}>{(evaluations.reduce((sum, evalItem) => sum + evalItem.rating, 0) / evaluations.length).toFixed(2)}</Text>
            <Text style={styles.statLabel}>平均评分</Text>
          </View>
          <View style={styles.statItem}>
            <Text style={styles.statValue}>{Array.from(new Set(evaluations.map(evalItem => evalItem.supplierId))).length}</Text>
